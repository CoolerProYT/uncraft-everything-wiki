...
| `experiences`    | Number | `1`       | Default experience required to uncraft an item. More detailed values can be configured in [`Per Item Exp Config`](/v1.8.2/per-item-exp-config). Must be > 0. |
...
See also [`FTB Quest Progression Config`](/v1.8.2/ftb-quest-progression-config).
:::
...
The config screen can be accessed by clicking the settings icon button in Uncrafting Table GUI:
...
::: tip
Common Config Screen button only available if the player is in creative mode or has Admin Permission
:::